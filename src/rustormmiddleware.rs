...
impl RustormMiddleware {
    pub fn new(db_url: &str) -> Self {
        RustormMiddleware { pool: ManagedPool::init(db_url, 5).unwrap() }
    }
}

impl Key for RustormMiddleware {
    type Value = Platform;
}
...
impl<D> Middleware<D> for RustormMiddleware {
    fn invoke<'mw, 'conn>(&self,
        ...
        req.extensions_mut()
           .insert::<RustormMiddleware>(self.pool.connect().unwrap());
        Ok(Continue(res))
    }
}
...
pub trait RustormRequestExtensions {
    ...
    fn orm_get<T: IsTable + IsDao>(&self,
                                   key: &str,
                                   val: &ToValue)
                                   -> Result<T, DbError>;
    fn orm_get_related<T: Entity, Src: Entity>(&self,
                                               src: &Src,
                                               rel_table: &str)
