...
fn main() {
    ...
    match diesel::update(users.filter(username.eq(&uname))).set(password.eq(&hashword)).execute(&db) {
        Ok(1) => { println!("Updated password"); }
        Ok(0) => {
            use rphotos::models::NewUser;
            diesel::insert(&NewUser {
                username: &uname,
                password: &hashword,
            }).into(users).execute(&db).expect("Create user");
            println!("New user");
        }
        Ok(n) => { println!("Strange, update {} users", n); }
...
fn random_password(len: usize) -> String {
    ...
    (0..len).map(|_| { match x.ind_sample(rng) {
